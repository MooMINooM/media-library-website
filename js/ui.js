...
export function renderSchoolInfo(info) {
    if (!info) return;
    // If passed as array, take first element
    if (Array.isArray(info)) { if (info.length === 0) return; info = info[0]; }
    ...
    // Header & Hero
    setText('header-school-name', info.school_name || '');
    setText('header-affiliation', info.affiliation || '');
    setText('hero-school-name-short', info.school_name || '');

    // Try optional hero elements (may not exist)
    setText('hero-motto', info.motto || '');
    const ageBadge = document.getElementById('school-age-badge');
    if (ageBadge && info.founding_date) {
        ageBadge.innerText = `ก่อตั้งมาแล้ว ${new Date().getFullYear() - new Date(info.founding_date).getFullYear()} ปี`;
    }
    const identityEl = document.getElementById('school-identity');
    if (identityEl && info.identity) { identityEl.innerText = info.identity; identityEl.classList.remove('hidden'); }

    // Logo
    const headerLogo = document.getElementById('header-logo');
    if (headerLogo && info.logo_url) { headerLogo.src = info.logo_url; headerLogo.classList.remove('hidden'); }
    const basicLogo = document.getElementById('header-logo-basic');
    if (basicLogo && info.logo_url) { basicLogo.src = info.logo_url; basicLogo.classList.remove('hidden'); const ph = document.getElementById('logo-placeholder'); if (ph) ph.classList.add('hidden'); }

    // School Basic Info
    setText('info-name-th', info.school_name || '-');
    setText('info-name-en', info.school_name_en || '-');
    setText('info-affiliation-val', info.affiliation || 'สังกัด...');
    setText('info-school-code', info.school_code_10 || '-');
    setText('info-smis-code', info.smis_code_8 || '-');
    setText('info-obec-code', info.obec_code_6 || '-');
    setText('info-address', info.address || '-');

    // School About
    setText('info-motto', info.motto ? `"${info.motto}"` : '"-"');
    setText('info-philosophy', info.philosophy ? `"${info.philosophy}"` : '"-"');
    setText('info-vision', info.vision || '-');
    setText('school-history-content', info.history || '-');
    setText('school-mission-content', info.mission || '-');
    setText('school-identity-content', info.identity || '-');
    setText('school-uniqueness-content', info.uniqueness || '-');

    // Color
    const colorBox = document.getElementById('school-color-box');
    if (colorBox) colorBox.style.background = `linear-gradient(to right, ${info.color_code_1 || info.color_code || '#ddd'} 50%, ${info.color_code_2 || info.color_code_1 || info.color_code || '#ddd'} 50%)`;

    // VTR
    const vtr = document.getElementById('vtr-iframe');
    if (info.vtr_url && vtr) {
        const vid = info.vtr_url.split('v=')[1]?.split('&')[0] || info.vtr_url.split('youtu.be/')[1];
        if (vid) { vtr.src = `https://www.youtube.com/embed/${vid}`; const ph = document.getElementById('vtr-placeholder'); if (ph) ph.style.display = 'none'; }
    }

    // Uniform
    const uniformImg = document.getElementById('student-uniform-img');
    if (uniformImg && info.uniform_url) { uniformImg.src = info.uniform_url; uniformImg.classList.remove('hidden'); const ph = document.getElementById('uniform-placeholder'); if (ph) ph.classList.add('hidden'); }

    // Music
    if (info.song_url) { const audio = document.getElementById('school-song'); if (audio) audio.src = info.song_url; const controls = document.getElementById('music-player-controls'); if (controls) controls.classList.remove('hidden'); }
    ...
    if (mapContainer && info.map_embed) {
        ...
        if (iframe) { iframe.className = 'w-full h-full absolute inset-0'; iframe.style.borderRadius = '2rem'; }
    }

    // Social
    const fbLink = document.getElementById('footer-facebook');
    if (fbLink && info.facebook) { fbLink.href = info.facebook; fbLink.classList.remove('hidden'); }
    const ytLink = document.getElementById('footer-youtube');
    if (ytLink && info.youtube) { ytLink.href = info.youtube; ytLink.classList.remove('hidden'); }

    // Footer
    setText('footer-school-name', info.school_name || '');
    const footerAff = document.getElementById('footer-affiliation');
    if (footerAff) footerAff.innerText = info.affiliation || '';
    const footerLogo = document.getElementById('footer-logo');
    if (footerLogo && info.logo_url) { footerLogo.src = info.logo_url; footerLogo.classList.remove('hidden'); }
}
...
export function renderStudentChart(data) { const container = document.getElementById('student-summary-container'); const chartCanvas = document.getElementById('studentChart'); if (!data || data.length === 0) { if (container) container.innerHTML = '<p class="text-center text-gray-400 col-span-3">ยังไม่มีข้อมูลนักเรียน</p>'; return; } data.sort((a, b) => a.id - b.id); let totalMale = 0, totalFemale = 0; data.forEach(d => { totalMale += parseInt(d.male || 0); totalFemale += parseInt(d.female || 0); }); if (container) { container.innerHTML = `<div class="info-card text-center"><div class="w-12 h-12 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-xl mx-auto mb-3"><i class="fa-solid fa-users"></i></div><p class="text-3xl font-bold text-blue-600">${totalMale + totalFemale}</p><p class="text-sm text-gray-500 font-medium mt-1">นักเรียนทั้งหมด</p></div><div class="info-card text-center"><div class="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-xl mx-auto mb-3"><i class="fa-solid fa-mars"></i></div><p class="text-3xl font-bold text-emerald-600">${totalMale}</p><p class="text-sm text-gray-500 font-medium mt-1">นักเรียนชาย</p></div><div class="info-card text-center"><div class="w-12 h-12 rounded-xl bg-pink-50 text-pink-600 flex items-center justify-center text-xl mx-auto mb-3"><i class="fa-solid fa-venus"></i></div><p class="text-3xl font-bold text-pink-600">${totalFemale}</p><p class="text-sm text-gray-500 font-medium mt-1">นักเรียนหญิง</p></div>`; } if (chartCanvas) { if (window.myStudentChart) window.myStudentChart.destroy(); window.myStudentChart = new Chart(chartCanvas, { type: 'bar', data: { labels: data.map(d => d.grade), datasets: [{ label: 'ชาย', data: data.map(d => d.male), backgroundColor: '#3b82f6', borderRadius: 6 }, { label: 'หญิง', data: data.map(d => d.female), backgroundColor: '#ec4899', borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true, grid: { display: false } }, y: { stacked: true, beginAtZero: true, grid: { color: '#f1f5f9' } } }, plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, padding: 20 } }, title: { display: true, text: 'แผนภูมิแสดงจำนวนนักเรียนแยกตามระดับชั้น', font: { size: 14, weight: 'bold' }, padding: { bottom: 20 } } } } }); } }
// --- Search/Filter Functions ---
window.filterAchievements = function (inputId, type, containerId) {
    const q = document.getElementById(inputId)?.value.toLowerCase().trim() || '';
    let allData;
    if (type === 'teacher') allData = allTeacherData;
    else if (type === 'student') allData = allStudentData;
    else allData = allSchoolData;
    if (!allData) return;
    const filtered = allData.filter(item => {
        const searchStr = [item.title, item.name, item.students, item.program, item.competition, item.date].filter(Boolean).join(' ').toLowerCase();
...
window.filterInnovations = function (inputId) {
    const q = document.getElementById(inputId)?.value.toLowerCase().trim() || '';
