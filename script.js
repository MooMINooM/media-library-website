...
// --- DROPDOWN, NAVIGATION, EVENT LISTENERS, MODAL ---
...
// --- 🌟 UPDATED: TEACHER ACHIEVEMENTS PAGE with corrected text clamping 🌟 ---
...
function renderTeacherAchievements(achievementsList) {
    ...
    achievementsList.forEach(item => {
        ...
        // NOTE: JavaScript truncation logic is removed. Truncation is now handled purely by CSS.

        card.innerHTML = `
            <div>
                <h4 class="font-bold text-blue-800 text-lg">${item.name || '-'}</h4>
                <a href="${item.url_pro || '#'}" target="_blank" rel="noopener noreferrer" 
                   class="block mt-1 text-sm text-gray-600 hover:text-blue-800 hover:underline line-clamp-2" 
                   title="${item.project || ''}">
                    ${item.project || '-'}
                </a>
            </div>
